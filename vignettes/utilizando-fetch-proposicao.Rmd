---
title: "Entendendo os parâmetros utilizados na função fetch_proposicao_camara()"
author: "Paulo Vinícius Soares"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Entendendo os parâmetros utilizados na função fetch_proposicao_camara()}
...
Na versão 0.2 do `rcongresso`, a função `fetch_proposicao_camara()` agora dá suporte à consultas mais complexas utilizando o endpoint `/proposicoes`. Para mais detalhes sobre os parâmetros, visite a página dos [Dados Abertos](https://dadosabertos.camara.leg.br/swagger/api.html).
...
- `siglaTipo`: Sigla do tipo da proposição, por exemplo PEC, REQ, PL, PLP, entre outros. Veja mais [detalhes](https://dadosabertos.camara.leg.br/api/v2/referencias/proposicoes/siglaTipo).
...
```{r}
fetch_proposicao_camara_camara(id = 2088351) %>%
  glimpse()
```
...
fetch_proposicao_camara_camara(ids) %>%
  glimpse()
```
...
```{r}
fetch_proposicao_camara_camara(dataApresentacaoInicio = "2017-01-01", dataApresentacaoFim = "2017-01-31") %>%
  glimpse()
```
...
```{r}
fetch_proposicao_camara_camara(dataInicio = "2017-11-01", dataFim = "2017-11-30") %>%
  glimpse()
```
...
```{r}
fetch_proposicao_camara_camara(siglaPartidoAutor = "PSC", itens = 100) %>%
  glimpse()
```
...
fetch_proposicao_camara_camara(siglaUfAutor = estados_nordeste, itens = 100) %>%
  glimpse()
```
...
tryCatch(
  fetch_proposicao_camara_camara(autor = autores) %>%
